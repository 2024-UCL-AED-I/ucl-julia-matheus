...
class Program
{
    static void Main()
    {
        ...
        SuperPoderes = JsonSerializer.Deserialize<List<SuperPoder>>(jsonString);

        SuperPoder poder = new SuperPoder();

        string superPoderesExemplo = "Força sobre-humana, agilidade aumentada e fator de cura acelerado";

        List<SuperPoder> poderes = SuperPoder.CriarSuperPoderes(superPoderesExemplo);

        poder.ImprimirListaDePoderes(poderes);
